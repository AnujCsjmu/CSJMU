...
{
    public class ExamCategoryRepository : BaseRepository, IExamCategoryRepository
    {
        public ExamCategoryRepository(IConfiguration configuration)
: base(configuration)
        { }
        ...
        public async Task<List<ExamCategoryModel>> GetExamCategoryAsync()
        {
            try
            {
                var query = "SP_InsertUpdateDelete_ExamCategory";       
                using (var connection = CreateConnection())
                {
                    DynamicParameters parameters = new DynamicParameters();
                    parameters.Add("@Query", 4, DbType.Int32);
                    var list = await SqlMapper.QueryAsync<ExamCategoryModel>(connection, query, parameters, commandType: CommandType.StoredProcedure);
                    return (List<ExamCategoryModel>)list;
                }
            }
            catch (Exception ex)
            {
