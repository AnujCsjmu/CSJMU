...
namespace CoreLayout.Repositories.Masters.Category
{
...
: base(configuration)
        ...
        public async Task<List<CategoryModel>> GetAllAsync()
        {
            try
            {
                var query = "SP_InsertUpdateDelete_Category";       
                using (var connection = CreateConnection())
                {
                    DynamicParameters parameters = new DynamicParameters();
                    parameters.Add("@Query", 4, DbType.Int32);
                    var list = await SqlMapper.QueryAsync<CategoryModel>(connection, query, parameters, commandType: CommandType.StoredProcedure);
                    return (List<CategoryModel>)list;
                }
            }
            catch (Exception ex)
            {
